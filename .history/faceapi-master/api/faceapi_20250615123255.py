...
def process_face_image(name, enrollment_id, image: Image.Image, use_tile_detection=False):
    ...
    if not embeddings_query:
        return f"No faces detected in query image for {name}."
    ...
    for i, embedding in enumerate(embeddings_query):
        ...
        if matched_name:
            ...
            attendance.save()
            results.append(f"Face {i+1}: {matched_name} (Similarity: {similarity:.4f})")
        else:
            # For unknown faces, we do not mark attendance (neither present nor absent).
            logging.info(f"Face {i+1} did not match any registered student.")
            results.append(f"Face {i+1}: No match found.")
    
    return {
    "matched": True,  # or False, depending on your logic
    "student_name": matched_name,  # or None if not matched
    "similarity": float(similarity)  # or None if not matched
}

def main(use_tile_detection=False):
    """
    Process query images for all students from MongoDB.
    For each query (group) image, compare all detected faces against the known database.
    Attendance is recorded only for registered students.
    Aggregated results are returned.
    """
    logging.info("Initializing Face Recognition Model for batch processing...")
    model = FaceAnalysis()
    model.prepare(ctx_id=0, det_size=(640, 640))
    
    logging.info("Fetching known faces from MongoDB for batch processing...")
    mongo_url = os.getenv('MONGO_URL')
    all_faces = get_all_students(mongo_url)
    logging.info(f"Number of known faces retrieved: {len(all_faces)}")
    ...
    for face in all_faces:
        ...
        if embeddings:
            ...
            profile_names.extend([face['name']] * len(embeddings))
            enrollment_numbers[face['name']] = face['enrollment_id']
            student_courses[face['name']] = face.get('course', 'Unknown')
            student_ids[face['name']] = face['_id']
    
    if not known_embeddings:
        logging.error("No known face embeddings were generated for index building.")
        return {"message": "Attendance marked successfully", "data": {"message": "No known faces available for comparison."}}
    
    index = build_faiss_index(known_embeddings)
    
    logging.info("Fetching query faces from MongoDB for batch processing...")
    query_faces = get_all_students(mongo_url)
    all_results = {}
    for face in query_faces:
        logging.info(f"Processing query for {face['name']}...")
        results = process_face_image(face['name'], face['enrollment_id'], face['decodedImage'],
                                     use_tile_detection=use_tile_detection)
        all_results[face['name']] = results
        logging.info(f"Results for {face['name']}: {results}")
    
    return {"message": "Attendance marked successfully", "data": {"message": all_results}}

if __name__ == "__main__":
    aggregated_results = main(use_tile_detection=USE_TILE_DETECTION)
    logging.info(f"Aggregated results: {aggregated_results}")
