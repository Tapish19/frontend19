  ...
  if (attendanceForm) {
    attendanceForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        const data = await res.json();
        console.log("Attendance Response:", data); // 🟡 For debugging

        // Extract face result from data.data.message[0]
        const faceResult = data?.data?.message?.[0]?.toLowerCase();

        if (faceResult && faceResult.includes("verified")) {
