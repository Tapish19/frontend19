...
export const markAttendance = async (file, req) => {
    try {
        const { id, name, course } = req.body;
        if (!file) {
            throw new Error("No file uploaded");
        }
        const image = file.buffer.toString('base64'); // Convert image buffer to base64
        ...
        // Throw error to be handled in the route
        throw error;
    }
};
